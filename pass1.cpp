...
void evaluateExpression(string expression, bool &relative, string &tempOperand, int lineNumber, ofstream &errorFile, bool &errorFlag)
{
...
void pass1()
{
    ...
    if (!checkCommentLine(fileLine))
    {
      ...
      else if (opcode == "ORG")
      {
        ...
        else
        {
          bool relative;
          errorFlag = false;
          evaluateExpression(operand, relative, tempOperand, lineNumber, errorFile, errorFlag);
          if (!errorFlag)
          {
  ...
  if (opcode == "END")
  {
    firstExecutableSection = SYMTAB[label].address;
    SYMTAB[firstExecutableSection].name = label;
    SYMTAB[firstExecutableSection].address = firstExecutableSection;
  }
